...
sub readPersistentLexicon($$$)
{
	...
	while (<$fh>)
	{
		...
		unless ( $line =~ m/^#/g )
		{
			...
			$tag   = $tlffLocal[0];
			$lemma =
			  normalise( $tlffLocal[1], $normaliseDiacritics, $normaliseCase );
			$form =
			  normalise( $tlffLocal[2], $normaliseDiacritics, $normaliseCase );
...
sub writeTrainLexicon($$)
{
	...
	open OF, ">$lexiconFile"
	  or die "Cannot open train lexicon output file $lexiconFile!\n";
...
sub readMassLexicon($$)
{
	...
	open IF, "$massLexiconFile"
	  or die "Cannot open mass lexicon input file $massLexiconFile!\n";
